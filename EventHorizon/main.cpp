...
//                space.isLaser = false;
        ...
        {
            BONUSES[i]->render(window);
            BONUSES[i]->animuj(elapsed);
            BONUSES[i]->out_of_screen(window.getSize());

            if(space.getGlobalBounds().intersects(BONUSES[i]->Sprite.getGlobalBounds()))
            {
                space.gain_points();
                BONUSES[i]->to_center(window.getSize());
            }
